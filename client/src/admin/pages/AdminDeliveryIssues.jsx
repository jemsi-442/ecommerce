...
export default function AdminDeliveryIssues() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return issueOrders.filter((order) => {
      ...
      } else if (issueFilter !== "all" && order.delivery?.issueStatus !== issueFilter) {
        return false;
      }
  ...
  }, [issueFilter, issueOrders, searchQuery]);
  ...
  const updateIssueStatus = async (orderId) => {
  ...
  if (loading) {
    return <PageState title="Loading delivery issues" description="Preparing the support queue..." />;
  }
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      ...
      {error ? <PageState tone="error" title="Delivery issues unavailable" description={error} /> : null}

      <section className="grid gap-3 md:grid-cols-4">
        <IssueMetric label="Reported issues" value={summary.total} tone="slate" />
        <IssueMetric label="Open" value={summary.open} tone="red" />
        <IssueMetric label="Investigating" value={summary.investigating} tone="orange" />
        <IssueMetric label="Resolved" value={summary.resolved} tone="emerald" />
      </section>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          <label className="block md:col-span-2">
            ...
            <input
              ...
              placeholder="Search by customer, rider, address, issue note, or proof"
            />
          </label>
      ...
      {!filteredOrders.length ? (
        <PageState
          ...
          description={issueOrders.length ? "Try another issue status or search term." : "Reported issues will appear here with proof, rider, and customer context."}
        />
      ) : (
                    ...
                    <div className="mt-4 space-y-3">
                      ...
                      <label className="block">
                        ...
                        <textarea
                          ...
                          placeholder="Write the support update the customer should rely on"
                        />
                      </label>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm text-slate-600">
                        ...
                        {order.delivery?.issueResolvedAt ? (
                          <p className="mt-1">
                            Resolved at <span className="font-semibold text-slate-900">{formatDateTime(order.delivery.issueResolvedAt)}</span>
                          </p>
                        ) : null}
                        {order.delivery?.issueResolutionNote ? (
                          <p className="mt-2 text-slate-700">{order.delivery.issueResolutionNote}</p>
                        ) : null}
                      </div>
...
function IssueMetric({ label, value, tone = "slate" }) {
  const toneMap = {
